...
import { assetStatusCopy, createBriefPayload } from "./frankWorkflow";
...
describe("frankWorkflow", () => {
  ...
  it("labels approval states with Frank voice", () => {
    expect(assetStatusCopy("approved")).toBe("Approved. Hot.");
    expect(assetStatusCopy("rejected")).toBe("Not the one.");
    expect(assetStatusCopy("review")).toBe("In review");
  });
});
